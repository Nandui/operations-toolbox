...
function buildSummaries(locations: TrailLocation[], rawReports: any[]): TrailTaskSummary[] {
  const today = new Date().toISOString().split('T')[0]
  const reportByLocation = new Map<string, any>()
  ...
  return locations.map((loc) => {
    ...
    return { locationId: loc.id, locationName: loc.name, date: today, totalTasks: total, completedTasks: completed, pendingTasks: pending, overdueTasks: overdue, completionRate: rate }
  })
}
...
export async function fetchTrailDashboardData(): Promise<TrailDashboardData> {
  ...
  const [locationsRes, reportsRes] = await Promise.all([
    trailFetch<any>('/locations', 3600),
    trailFetch<any>(`/task-reports?date=${today}`, 300),
  ])
  const locations: TrailLocation[] = Array.isArray(locationsRes) ? locationsRes : (locationsRes.data ?? locationsRes.locations ?? [])
  const rawReports: any[] = Array.isArray(reportsRes) ? reportsRes : (reportsRes.data ?? reportsRes.reports ?? [])
  const summaries = buildSummaries(locations, rawReports)
  const totals = computeTotals(summaries)
  return { fetchedAt: new Date().toISOString(), locations, summaries, totals }
}
...
export function getMockTrailData(): TrailDashboardData {
  ...
  return {
    ...
    locations,
    summaries,
    totals: { totalTasks: 72, completedTasks: 61, pendingTasks: 8, overdueTasks: 3, overallCompletionRate: 85 },
  }
}
