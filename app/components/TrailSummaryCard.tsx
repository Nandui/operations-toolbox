...
export default function TrailSummaryCard({ label, value, color = 'default' }: { label: string; value: string | number; color?: Color }) {
  return (
    <div className={`rounded-xl p-5 shadow-sm border border-slate-100 ${colorMap[color]}`}>
      ...
      <p className={`mt-1 text-3xl font-bold tabular-nums ${valueColorMap[color]}`}>{value}</p>
    </div>
  )
}
