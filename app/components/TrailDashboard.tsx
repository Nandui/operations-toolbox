  ...
  try {
    ...
    return (
      <section className="rounded-xl border border-red-100 bg-red-50 p-6">
        ...
        <p className="mt-1 text-sm text-red-600">Could not connect to Trail API. Check your API key and try again.</p>
      </section>
    )
  }
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <TrailSummaryCard label="Completion Rate" value={`${totals.overallCompletionRate}%`} />
        <TrailSummaryCard label="Tasks Complete" value={totals.completedTasks} color="green" />
        <TrailSummaryCard label="Pending" value={totals.pendingTasks} color="amber" />
        <TrailSummaryCard label="Overdue" value={totals.overdueTasks} color="red" />
      </div>
      {summaries.length > 0 && (
        <div className="mt-6 bg-white rounded-xl border border-slate-100 shadow-sm divide-y divide-slate-100 px-6">
          {summaries.map((s) => <TrailLocationRow key={s.locationId} summary={s} />)}
        </div>
      )}
      <p className="text-xs text-slate-400 mt-3">
        {useMock ? 'Add TRAIL_API_KEY to .env.local to load live data' : `Last updated: ${formatTime(fetchedAt)} · refreshes every 5 min`}
      </p>
    </section>
  )
}
